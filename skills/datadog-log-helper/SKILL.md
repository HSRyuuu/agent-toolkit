...
description: >
  ...
  deploy regressions, remember frequently used Datadog log queries, or manage
  local Datadog log access context. Triggers: "Datadog 로그", "datadog api",
  "서비스 에러 로그", "로그 집계", "몇 건이야", "장애 타임라인", "배포 후 에러",
  and in Datadog log context: "기억해", "기억해줘", "memory", "MEMORY".
---

# Datadog Log Helper

This skill explores Datadog logs through the Datadog Logs API, avoiding MCP and
returning compact, pre-trimmed output. It is read-first: use it for local API key
setup, log searches, error investigation, timeline reconstruction, and maintaining
the user's Datadog log access memory.

Keep this file as the router. For any real task, first read
`~/.config/datadog-log-helper/MEMORY.md` if it exists and honor saved service,
...
`~/.config/datadog-log-helper/` contains two local files:
...
저장할 때는 내용 성격에 맞는 섹션(서비스 별칭 / 로그 접근 / 로그 스키마 / 자주
쓰는 쿼리 / 선호)을 고른다. 로그 원문, 고객 데이터, 키/secret은 어떤 경우에도
저장하지 않는다.
