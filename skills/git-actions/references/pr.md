...
> `git-actions`의 **PR 생성** / **PR 파일 모드** 작업이 따르는 프롬프트.
> 현재 브랜치의 변경사항을 묶어서 PR/MR 본문을 만든다. **GitHub이면 자동 생성, 그 외에는 markdown 파일로 떨궈서 수동 생성.**

## 핵심 원칙

- **자동 생성은 GitHub만** — `gh pr create` 한 가지 경로만 자동화한다. GitLab(`glab`)이나 다른 호스트는 분기하지 않고 일괄적으로 파일 모드로 보낸다 (단순화 목적).
...
- **PR 전에 베이스 최신화** — PR을 만들기 전에 베이스 브랜치를 머지해 충돌을 먼저 해소한다. 충돌 분류·해결은 `resolve-conflict.md`를 따른다.
...
- 워킹 트리 dirty면 사용자에게 알리고 커밋 여부 확인 (`commit.md` 권유).
...
```
- 종료 코드 `0` → 충돌 없음. 클린 머지로 진행:
  ```bash
  git merge --no-edit "origin/$BASE"
  ```
  ("Already up to date"면 아무 일도 일어나지 않음.)
- 종료 코드 `1` → 충돌 있음. `git merge --no-edit "origin/$BASE"`로 머지를 시작한 뒤, **`resolve-conflict.md`의 분류·해결 절차를 따른다.** 모호한 충돌이 하나라도 남아 있으면 PR 생성을 진행하지 않고 대기한다.
  (`git merge-tree`가 없는 구버전 git이면 곧바로 머지를 시도하고 충돌 시 `resolve-conflict.md`로.)

워킹 트리가 dirty면 머지 전에 멈추고 사용자에게 알린다 (커밋/stash는 임의로 하지 않는다 — 1단계 정책과 동일).

> 머지 커밋이 생겼으므로 6단계 푸시 시 upstream에 반영해야 PR에 포함된다.
...
- 기존 커밋 스타일을 따른다 (`commit.md`의 Style Detection — 한국어/영어, semantic prefix 여부 감지).
...
4. 베이스 머지 중 **모호한 충돌** → 처리 방법 (`resolve-conflict.md` 참조)
...
| 베이스 머지 시 자명한 충돌(lockfile/import/포매팅) | `resolve-conflict.md`의 SAFE 기준으로 직접 해결 후 `git add` → 머지 커밋 |
| 베이스 머지 시 모호한 충돌(같은 로직 양쪽 수정) | 추측 금지 — `resolve-conflict.md`의 ESCALATE로 사용자에게 질문 |
