...
3. **수정·제거는 사용자가 명시할 때만** — 입력에 "수정/변경/바꿔/업데이트/교체/rotate/갱신" 같은 동사가 있을 때만 modify 모드. "삭제/제거/지워/빼줘"가 있을 때만 remove 모드. 그 외에는 silent overwrite 절대 금지.
...
1. `{kb_root}/00_Inbox/`의 파일 목록을 스캔.
2. 비어있으면 안내 후 종료:
   > "00_Inbox/가 비어있습니다. 정리할 항목이 없습니다."
3. 파일 1개씩 다음을 수행:
   a. 파일 내용을 읽고 1~3문단으로 요약 추출.
   b. 파일 타입 따라 Step 2A/2B/2C와 동일하게 처리하되, **raw/ 복사는 건너뜀** (이미 볼트 안에 있음).
   c. Step 3·4·5·6·7을 그대로 진행.
   d. 컨펌 시 추가 옵션 `s/skip` 제공 — 이 파일은 00_Inbox에 그대로 남김.
   e. 승인 후 쓰기 완료되면 **원본 00_Inbox 파일은 삭제** (다른 곳에 이미 정리되었으므로).
...
| 카테고리 | 동사 (입력 본문에 명시되어야 함) | 분기 |
|---|---|---|
| **modify** | "수정", "변경", "바꿔", "업데이트", "교체", "rotate", "갱신" | → Step 3-M |
| **remove** | "삭제", "제거", "지워", "빼줘" | → Step 3-R |
| (없음) | 그 외 | → Step 3-1 (신규/append) |

modify·remove는 **반드시 대상 파일이 지정되어야** 한다 (`[[creds]]`, `credentials.md` 등). 대상이 모호하면 사용자에게 1회 질문:
> "어느 파일을 수정할까요? (예: `[[credentials]]`)"

대상 미지정 상태로는 modify/remove를 진행하지 않는다.
...
3. **diff preview**를 출력:
   ```
   === 수정 계획: 03_Resources/credentials.md ===
...
- ❌ 사용자가 명시적으로 modify/remove 동사를 쓰지 않은 한 **기존 본문을 한 줄도 수정·삭제하지 않는다.**
...
- ❌ 5개 문서 자동 cascade append — 그건 `/kb-link`의 책임.
