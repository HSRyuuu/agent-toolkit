...
3. **agent-toolkit 스킬만 다룬다.** `${CLAUDE_PLUGIN_ROOT}/skills/` 디렉토리만 스캔한다. 외부 플러그인이나 사용자 글로벌 스킬은 대상이 아니다.
...
`skills/` 아래의 모든 `SKILL.md`에서 frontmatter를 읽는다.
...
- `skills/create-claude-plugin` 참고 (플러그인 자체를 새로 만들 때)
