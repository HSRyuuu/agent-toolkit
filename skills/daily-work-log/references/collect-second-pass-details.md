...
참조 표기 규칙 (SKILL.md의 "참조 표기 규칙"과 동일):

- git repo로 관리되는 소스코드는 full path를 쓰지 않는다. 코드 경로는 `` `repo-name` git repository ``, 파일은 `` `repo-name` `FileName.java` `` 형태로 짧게 쓴다.
- 로컬에만 있어 local path 없이는 다시 찾을 수 없는 파일(개인 메모, 계획서 초안 등)만 절대 경로(full path)로 쓴다.
- 인터넷 링크는 `링크 명(요약): URL` 형태로 쓴다.
- repo 이름, 파일명, 명령어, 코드 식별자는 백틱(`인라인 코드`)으로 감싼다.
