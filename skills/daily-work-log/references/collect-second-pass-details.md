...
이 skeleton은 초안이다. LLM은 `second-pass-digest.json`과 필요한 최소 근거만 확인해 `summary`, `why_it_mattered`, `decisions`, `learnings`, `troubleshooting`, `follow_ups`, `uncertainties`를 보강한다. 스크립트가 채운 `what_i_did`, `technical_context`, `evidence_paths`는 원문 복사가 아니라 검토용 후보이므로, 최종 Markdown 작성 전에 민감정보와 과도한 세부사항을 다시 걸러낸다.
...
```json
{
  "selected_candidates": [
    {
      "group_id": "4",
      "title": "metatrip-api Config Server / Spring profile 구조 조사",
      "list_section": "top",
...
최종 Markdown에는 긴 원문 발췌보다 경로 모음을 우선 남긴다.

최종 Markdown은 frontmatter와 `# YYYY-MM-DD 업무 기록` 제목 다음에 바로 `## 한눈에 보기`를 둔다. `## 한눈에 보기`에는 선택된 작업 목록을 `| 구분 | 제목 | 요약 |` 표로 요약한다. 별도의 `## 작업 요약` 섹션은 만들지 않고, 표 바로 아래에 각 선택 항목을 `### 항목 제목`으로 작성한다.

각 `###` 항목은 사용자가 나중에 제목, 표 요약, 자신의 기억만으로도 맥락과 처리 내용을 복원할 수 있게 쓴다. 관련 링크, 코드 경로, 관련 문서, 세션/근거처럼 참조를 모으는 목록은 2-depth list까지 허용한다. 그 외 본문은 불필요하게 깊은 중첩 목록을 만들지 않는다.

최종 Markdown 맨 아래에는 반드시 `<!-- 이 아래는 자유 형식 -->` 주석 마커를 둔다. 앞의 고정 구조에 맞지 않지만 사용자가 남기고 싶은 메모, 추가 회고, 임시 생각은 이 마커 아래에 작성한다.

권장 근거:

- 세션 파일 경로와 session id
- KB 문서 경로
- 사용자가 직접 작성한 계획/회의/트러블슈팅 문서 경로
- 확인한 코드 파일 경로
...
사용자가 승인하면 템플릿을 적용해 최종 Markdown 초안을 작성한다. 최종 저장 경로는 journal root 바로 아래가 아니라 `daily-work-log/` 하위다.

최종 Markdown의 frontmatter에서 `type` 허용값은 `daily-work-log`, `decision-record`, `trouble-shooting`, `learning-note` 네 가지만 사용한다. 이 단계에서 작성하는 일일 업무 기록의 `type`은 `daily-work-log`로 둔다. `tags`는 빈 배열로 두지 말고, 문서 작성을 마친 뒤 "나중에 이 문서를 검색한다면 어떤 키워드로 찾을까?"를 기준으로 에이전트가 판단해 충분히 채운다. 태그 값은 고정 목록으로 제한하지 않으며, `daily-work-log`를 의무적으로 넣지 않는다.

```text
<journal-root>/daily-work-log/YYYY/MM/YYYY-MM-DD.md
```

예를 들어 journal root가 `~/personal`이면 `~/personal/daily-work-log/2026/07/2026-07-02.md`에 작성한다.
