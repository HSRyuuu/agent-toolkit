...
1차 수집 결과물은 기본적으로 `~/.daily-work-log/` 아래에 저장한다.
...
이 디렉터리는 도구 상태와 low-level evidence 보관 위치다. 최종 Markdown 업무 기록은 사용자의 journal root, 예를 들어 `~/personal/daily-work-log/`, 아래에 작성한다.
...
```bash
python3 skills/daily-work-log/scripts/collect_codex_first_pass_candidates.py --date YYYY-MM-DD
```

stdout으로 확인만 할 때:

```bash
python3 skills/daily-work-log/scripts/collect_codex_first_pass_candidates.py --date YYYY-MM-DD --stdout
```

스크립트가 수행하는 일:

- `~/.codex/sessions/YYYY/MM/DD/*.jsonl`을 찾는다.
...
```bash
python3 skills/daily-work-log/scripts/collect_claude_first_pass_candidates.py --date YYYY-MM-DD
```

stdout으로 확인만 할 때:

```bash
python3 skills/daily-work-log/scripts/collect_claude_first_pass_candidates.py --date YYYY-MM-DD --stdout
```
...
```bash
python3 skills/daily-work-log/scripts/collect_kb_first_pass_candidates.py --date YYYY-MM-DD
```

stdout으로 확인만 할 때:

```bash
python3 skills/daily-work-log/scripts/collect_kb_first_pass_candidates.py --date YYYY-MM-DD --stdout --emit-empty
```
...
- KB의 `log.jsonl`에서 대상 날짜 기록을 찾는다.
...
- 원시 JSONL을 먼저 읽지 않는다.
- 먼저 `~/.daily-work-log/first-pass/YYYY-MM-DD/*-candidates.json`만 읽는다.
...
3. `work_units`를 사람이 읽을 수 있는 주제 단위로 묶어 “상위 후보 목록” 10개까지 제안한다.
4. 상위 후보 목록에 들어가지 않았지만 쓸만한 후보가 있으면 “기타 후보 목록”에 최대 3개까지만 제안한다.
5. 마지막에는 사용자가 고를 수 있게 “어떤 묶음을 오늘 일지에 포함할까요?”라고 묻는다.
...
결과 파일:
- Codex: `~/.daily-work-log/first-pass/2026-07-02/codex-candidates.json`
- Claude: `~/.daily-work-log/first-pass/2026-07-02/claude-candidates.json`
- KB: `~/.daily-work-log/first-pass/2026-07-02/kb-candidates.json`
...
상위 후보 목록:
Codex
- user-module - 사용자 도메인 구조와 레이어 책임 정리, 문서/수정 신호
- event-worker - Kafka 연동 문제 원인 확인과 재처리 흐름 점검, 에러/테스트 신호
- admin-ui - 목록 필터 동작 규칙 개선, 수정/git/테스트 신호
- local-dev - 패키지 매니저 버전과 로컬 실행 방법 정리, 설정/검증 신호

Claude
- docs-vault - 업무 문서 분류 기준 정리, 문서 수정 신호
- support-note - 회의 메모와 후속 요청사항 정리, 업무 기록 신호
- auth-flow - 로그인 흐름 질문 정리와 참고 자료 확인, 조사 신호

기타 후보 목록:
- batch-job - 배치 실행 조건 확인, 낮은 confidence
- onboarding-note - 참고 자료 위치 확인, supporting 후보
```

KB 파일이 생성되지 않았거나 비어 있으면 예시의 KB 결과 파일과 수집량 줄은 표시하지 않는다.

`기타 후보 목록`에는 상위 후보와 중복된 항목을 넣지 않는다. `<turn_aborted>`만 남은 후보, `local-command-*` 로그인/훅 확인 노이즈, `Continue from where you left off` 같은 단독 재개 신호는 기본적으로 숨긴다.
