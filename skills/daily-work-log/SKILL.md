...
현재 등록된 항목: [ Jira Ticket, PR/MR 기록, ... ]
지금 항목을 추가/수정/삭제 할 수 있습니다.
```

- 현재 등록된 항목은 `~/.daily-work-log/config.json`의 `impact-items` 값을 그대로 보여준다. 값이 없거나 빈 배열이면 "현재 등록된 항목: 없음"으로 안내한다.
- 사용자가 추가/수정/삭제를 요청하면, 2차 상세 탐색을 시작하기 전에 `config.json`의 `impact-items` 배열을 먼저 갱신한다(다른 필드는 유지). 갱신 후에는 바뀐 목록을 한 줄로 다시 확인해 준다.
...
`~/.daily-work-log/config.json`의 `impact-items`는 사용자가 매번 최종 Markdown 맨 위에서 바로 확인하고 싶은 항목을 직접 정의하는 문자열 배열이다. 예: `["Jira Ticket", "PR/MR 기록", "최종 개발 결정 사항"]`.
...
- 섹션 안 라벨과 순서는 `impact-items` 배열의 문자열과 순서를 그대로 사용한다.
...
- 추출은 1차/2차 수집 스크립트에 새 정규식 신호를 추가하지 않는다. 2차 상세 탐색에서 LLM이 선택 후보의 원문을 읽으며 `impact-items` 각 라벨에 해당하는 근거가 있는지 함께 확인해서 채운다. 근거가 없으면 지어내지 않고 빈 채로 둔다.
...
```json
{
  ...
  "impact-items": ["Jira Ticket", "PR/MR 기록", "최종 개발 결정 사항"]
}
```
