# Datadog Helper Setup Guide

Use this file when config is missing, keys are invalid, Datadog site is unknown,
or log/APM search returns authentication/permission errors.
...
- API key
- Application key
- 조회 권한. scoped application key를 쓰는 경우 아래 스코프가 최소셋이다.
  - 로그 조회: `logs_read_data`, `logs_read_index_data`
  - APM 조회: `apm_read`
  - `timeseries_query`(메트릭 조회)는 주지 않는다. 스팬 집계로 latency percentile까지
    계산되므로 필요하지 않고, 조직 전체 메트릭 읽기 권한이 열려 최소 권한 원칙에 어긋난다.

## 진행 방식

아래 1~4단계를 순서대로 진행한다. 이미 완료된 단계는 검증 후 건너뛴다.
APM 권한 오류만 해결하러 왔다면 4단계의 `apm-test`부터 실행한다.
...
scoped application key를 만들 수 있다면 이 세 가지 읽기 스코프만 주는 쪽이 좋아요.
- logs_read_data, logs_read_index_data (로그 조회)
- apm_read (APM 트레이스/스팬 조회)
...
`~/.config/datadog-helper/config.json`에 저장되고 파일 권한은 `600`으로
맞춰진다. (예전 이름으로 만든 `~/.config/datadog-log-helper/config.json`도
그대로 인식한다.)
...
APM도 쓸 계획이면 스팬 조회 권한까지 확인한다. 403/forbidden이 나오면
application key에 `apm_read` 스코프가 없는 것이다.

```bash
python3 "<SKILL_DIR>/scripts/datadog_setup.py" apm-test --profile default
```

성공하면 이렇게 마무리한다.

```text
Datadog 조회 설정이 끝났어요. 이제 이런 식으로 물어보시면 됩니다.

- payments-api 최근 에러 로그 봐줘
- prod에서 30분 동안 500 에러가 늘었는지 봐줘
- 배포 이후 에러 타임라인 만들어줘
- payments-api에서 제일 느린 API가 뭔지 p95로 봐줘
- 이 trace_id로 어디서 시간이 오래 걸렸는지 봐줘
- 이 서비스 접근 방법 MEMORY.md에 저장해줘
```
