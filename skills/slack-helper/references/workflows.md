...
공통 규칙: 결과가 100건을 넘을 것 같으면 `--count` 대신 `--limit N`을 쓴다. 집계·통계가 필요하면 `references/adhoc-scripts.md`를 따른다. 검색 결과에서 스레드를 열 때는 결과 라인의 permalink를 그대로 `thread --permalink`로 넘긴다 (`p...` 숫자를 직접 ts로 변환하지 않는다).
...
2. Open only threads that look actionable, using each result line's permalink.
...
2. Read channel history only for likely source channels. 하루면 `--on`, 여러 날에 걸친 장애면 `--after/--before`(양끝 날짜 포함)를 쓴다.

```bash
python3 "<SKILL_DIR>/scripts/slack_read.py" channel-history --channel backend --on 2026-07-01 --limit 100
python3 "<SKILL_DIR>/scripts/slack_read.py" channel-history --channel backend --after 2026-07-01 --before 2026-07-02 --limit 200
```
...
2. Open only threads with real conversation from step 1, using each result line's permalink.
