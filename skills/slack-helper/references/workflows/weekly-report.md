...
```bash
python3 skills/slack-helper/scripts/slack_search.py search deploy review decision --from me --days 7 --count 50
```

3. Narrow by major channel when needed.

```bash
python3 skills/slack-helper/scripts/slack_search.py search deploy --from me --in backend --days 7
```
