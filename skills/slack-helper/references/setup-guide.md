...
2. 같은 화면을 아래로 내려 **Scopes(범위)** — 이 앱이 Slack에서 무엇을 읽을 수 있는지 정하는 권한 목록:
   ...
   - **사용자 토큰 범위(User Token Scopes)**의 `Add an OAuth Scope`에서 아래 5개를 하나씩 추가한다.
...
이 단계에서 아래 안내 문구를 반드시 함께 전달한다.

> 이 스킬은 여기서 등록한 권한까지만 Slack을 조회할 수 있어요. 위 목록은 전부 "읽기" 권한이라, 메시지를 보내거나 수정·삭제하는 일은 이 스킬로는 불가능합니다. 반대로, 여기에 추가하지 않은 권한이 필요한 조회(예: DM 읽기)는 안 돼요.
