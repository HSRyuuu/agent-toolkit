...
- `skills/kb-common/` - `kb-*` 스킬이 공유하는 LLM wiki schema/template/helper 자산. `SKILL.md`가 없어 플러그인 스킬로 로드되지 않음
...
`SKILL.md` 없는 보조 디렉토리(예: `skills/kb-common/`)는 공유 자산으로만 취급하고 inventory에 포함하지 않는다.
