...
Codex는 `.agents/plugins/marketplace.json`과 `.codex-plugin/plugin.json`을 사용한다. Codex manifest는 단일 스킬 루트인 `skills/`만 등록하므로, `skills-system/`과 `forks/taste-skill/`의 스킬은 `skills/` 아래 symlink로 함께 노출한다.
...
- `forks/taste-skill/` — 외부 taste skill 모음
...
Codex 호환을 위해 `skills-system/`과 `forks/taste-skill/`의 스킬도 `skills/` 아래 symlink로 노출한다. 아래 표는 원본 `skills/` 디렉토리의 직접 보관 스킬만 적는다.
