...
| `s/update-project-docs` | `update-project-docs` | `.claude/`의 작업 문서를 코드베이스 실제 상태와 비교하여 드리프트를 동기화. | 선택 |

`manage-skills`/`verify-implementation`은 짝으로 동작한다 — 생성한 `verify-*`를 통합 실행한다. `update-project-docs`는 독립적으로 동작한다.

### 작업 문서 (선택 시 `.claude/`에 설치, CLAUDE.md만 예외적으로 루트도 선택 가능)

코드와 동기화되어야 하는 문서. `update-project-docs` 스킬의 검증 대상.

| 키 | 문서 | 역할 | 기본 권장 |
|----|------|------|---------|
| `d/CLAUDE.md` | `CLAUDE.md` | 프로젝트 진입점 — 도메인 규칙·문서 매핑·정보 소유권 | **필수 권장** |
| `d/PROJECT_OVERVIEW.md` | `PROJECT_OVERVIEW.md` | 프로젝트 정체성·기술 스택·마일스톤·핵심 링크 | **필수 권장** |
| `d/SOURCE_MAP.md` | `SOURCE_MAP.md` | 소스코드 위치 라우팅 맵 | 선택 |
| `d/DB_SCHEMA.md` | `DB_SCHEMA.md` | DB 테이블·컬럼·제약·접근 범위 | 선택 |
| `d/DEPLOY.md` | `DEPLOY.md` | 인프라·도메인·환경변수·롤백 절차 | 선택 |
| `d/DESIGN.md` | `DESIGN.md` | 디자인 시스템 — 색상 팔레트·타이포·컴포넌트 스타일·다크 모드 | 선택 |
...
### 코딩 규칙 (선택 시 `.claude/rules/`에 설치)

`templates/rules/` 디렉토리에 있는 파일을 동적으로 스캔하여 사용자에게 한 파일씩 권유한다. **하드코딩하지 않는다.**
...
- **Rules 디렉토리 (선택)**: `${CLAUDE_PLUGIN_ROOT}/templates/rules/` — 코딩 규칙·스타일 가이드 파일 모음. 디렉토리가 없거나 비어있으면 rules 단계 자체를 건너뛴다.
...
| `PROJECT_OVERVIEW.md` | `$TEMPLATE_DIR/PROJECT_OVERVIEW.md` | `$TARGET/.claude/PROJECT_OVERVIEW.md` | 필수 |
| `SOURCE_MAP.md` | `$TEMPLATE_DIR/SOURCE_MAP.md` | `$TARGET/.claude/SOURCE_MAP.md` | 선택 |
| `DB_SCHEMA.md` | `$TEMPLATE_DIR/DB_SCHEMA.md` | `$TARGET/.claude/DB_SCHEMA.md` | 선택 |
| `DEPLOY.md` | `$TEMPLATE_DIR/DEPLOY.md` | `$TARGET/.claude/DEPLOY.md` | 선택 |
| `DESIGN.md` | `$TEMPLATE_DIR/DESIGN.md` | `$TARGET/.claude/DESIGN.md` | 선택 |
...
**Rules 디렉토리는 별도 단계(Step 7)에서 동적으로 처리한다.**
...
이용 가능한 자산:
  ...
  [스킬]  (.claude/skills/ 아래에 설치)
    ...
    3) update-project-docs    — .claude/ 작업 문서를 코드와 동기화

  [작업 문서]  (.claude/ 아래에 설치, CLAUDE.md만 루트도 선택 가능)
    4) CLAUDE.md              — 진입점·도메인 규칙·문서 매핑           [필수 권장 ✓]
    5) PROJECT_OVERVIEW.md    — 프로젝트 정체성·기술 스택·마일스톤    [필수 권장 ✓]
    6) SOURCE_MAP.md          — 소스코드 라우팅 맵
    7) DB_SCHEMA.md           — DB 테이블·컬럼·접근 범위
    8) DEPLOY.md              — 인프라·도메인·환경변수·롤백
    9) DESIGN.md              — 디자인 시스템·색상·타이포·컴포넌트

  [자기개선]  (.claude/ 아래에 설치, 에이전트가 자동 누적)
   10) LESSONS.md             — 사용자 피드백 누적, 자기개선 순환
```
...
각 SELECTED 자산이 대상에 이미 존재하는지 확인한다. **CLAUDE.md는 양쪽 위치(루트 / .claude/)를 모두 검사**한다 (3-1 케이스).
...
> `.claude/`의 git 포함 여부는 프로젝트 정책이므로 임의로 `.gitignore`를 수정하지 않는다.
...
### Step 7 — Rules 인터랙티브 선택 (`templates/rules/` 동적 스캔)

> **하드코딩 금지 — 이 단계의 파일 목록은 SKILL.md에 박아두지 않는다.** 런타임에 `RULES_DIR`을 스캔하여 사용자에게 한 파일씩 권유한다.

#### 7a. 디렉토리 스캔

```bash
if [ ! -d "$RULES_DIR" ]; then
  echo "rules 디렉토리가 없습니다 — rules 단계를 건너뜁니다."
  # Step 8로 건너뛴다
fi

mapfile -t RULE_FILES < <(find "$RULES_DIR" -maxdepth 1 -type f -name "*.md" | sort)

if [ ${#RULE_FILES[@]} -eq 0 ]; then
  echo "rules 디렉토리에 적용 가능한 규칙이 없습니다."
  # Step 8로 건너뛴다
fi
```
...
for f in "${RULE_FILES[@]}"; do
  ...
  SUMMARY=$(awk '/^[^#>]/{print; exit}' "$f" | head -c 200)
done
```

#### 7c. 인터랙티브 선택

먼저 **전체 목록을 한 번 보여준 뒤**, 각 파일마다 `AskUserQuestion`으로 묻는다:

```
이용 가능한 코딩 규칙 (templates/rules/):

  - <file-1>.md  — <첫 # 헤더 텍스트>
  - <file-2>.md  — <첫 # 헤더 텍스트>
  ...

각 규칙을 하나씩 검토하면서 적용 여부를 결정합니다.
```

각 파일에 대해:

```
[i/N] <rule-filename>.md
  제목: <첫 # 헤더에서 추출>
  요약: <첫 단락 미리보기>

  → 이 규칙을 이 프로젝트에 적용하시겠습니까?
    1. 네, 적용 (`<TARGET>/.claude/rules/<rule-filename>.md`로 복사)
    2. 미리보기 더 보기 (전체 본문 출력 후 다시 묻기)
    3. 건너뛰기 (이번엔 적용 안 함)
    4. 이후 모든 규칙도 자동으로 건너뛰기 (남은 항목 일괄 스킵)
```

#### 7d. Rules 설치 — 충돌 시 묻기 (대전제 1)
...
for rule in "${SELECTED_RULES[@]}"; do
  SRC="$RULES_DIR/$rule"
  DEST="$TARGET/.claude/rules/$rule"

  if [ -f "$DEST" ]; then
    # 대전제 1 — 묻지 않고 덮어쓰지 않는다
    # AskUserQuestion으로 다음 중 선택받음:
    #   1. 그대로 두기 (이 규칙은 이번엔 설치 안 함)
    #   2. 백업 후 교체 (.bak.<timestamp>)
    #   3. 취소
    # 결정에 따라 처리
    continue # 또는 백업 후 cp
  fi
...
**대전제 1 — 묻지 않고 덮어쓰지 않는다.** 기존 rules 파일이 있으면 처리 방식을 사용자에게 묻고 결정한다.

### Step 8 — CLAUDE.md 문서 매핑 채우기
...
| `/update-project-docs` | .claude/ 작업 문서를 코드베이스 실제 상태와 비교해 드리프트 동기화 |
```

#### 8b. 외부문서 표 채우기

설치된 문서(CLAUDE.md 자기 자신은 제외) 각각에 대해 한 행을 추가한다:

| 문서 | 용도 |
|---|---|
| `.claude/<key>` | <한 줄 설명> |

설치된 rules 파일이 있으면 같은 표에 `.claude/rules/<rule>.md` 행도 함께 추가한다. LESSONS.md를 설치했으면 외부문서 표에 함께 등록한다 (자기개선 문서지만 진입 정보로서 가치가 있음).

예시 — `SOURCE_MAP.md` + `DB_SCHEMA.md` + 임의의 룰 한 개(`<rule>.md`)를 설치한 경우:

```markdown
| 문서 | 용도 |
|---|---|
| `.claude/SOURCE_MAP.md` | 소스코드 위치 라우팅 맵 |
| `.claude/DB_SCHEMA.md` | DB 테이블·컬럼·접근 범위 |
| `.claude/rules/<rule>.md` | <rule 파일의 첫 # 헤더 텍스트> |
```

#### 8c. 템플릿 안내 주석·placeholder 행 제거 (대전제 2)
...
- **`/update-project-docs`** — 코드와 어긋난 `.claude/` **작업 문서**(CLAUDE.md, PROJECT_OVERVIEW.md, SOURCE_MAP.md, DB_SCHEMA.md, DEPLOY.md, DESIGN.md 중 설치한 것)를 코드 기준으로 동기화. **LESSONS.md는 동기화 대상이 아니다** — 에이전트가 사용자 피드백을 받을 때마다 직접 누적한다.
- **`.claude/LESSONS.md`** — 매 세션 시작 시 에이전트가 가장 먼저 읽고, 사용자 피드백이 들어오면 즉시 형식에 맞춰 추가한다.
- **`.claude/rules/<rule>.md`** — 작업 영역에 해당하는 규칙 파일을 그때그때 참조. CLAUDE.md "외부문서" 표를 통해 발견.

이 스킬(`project-setup`)은 위 사이클이 시작될 수 있도록 **골격만 까는** 역할에 한정된다. 프로젝트별 `verify-*` 스킬은 만들지 않는다 — 그건 `/manage-skills`의 책임 영역이다. 규칙 본문은 작성하지 않는다 — `templates/rules/`에 있는 것을 복사할 뿐이다.
...
6. **rules 디렉토리가 비어있거나 없음** — Step 7을 건너뛴다. 오류 아님.
7. **카탈로그의 일부 자산 누락** — 템플릿 폴더에 일부 파일이 없으면 메뉴에서 제외하고 진행. 카탈로그가 완전히 비어야만 중단한다.
8. **CLAUDE.md가 양쪽 위치(루트/.claude/)에 동시 존재** — Step 4a에서 사용자에게 묻고 정리.
...
| `templates/rules/` | 적용 가능한 코딩 규칙 파일들이 있는 디렉토리. Step 7에서 동적으로 스캔하여 사용자에게 제시. 특정 파일을 하드코딩으로 가정하지 않는다. |
...
| `<target>/.claude/PROJECT_OVERVIEW.md` | (선택) 설치된 프로젝트 개요 골격 |
| `<target>/.claude/SOURCE_MAP.md` | (선택) 설치된 라우팅 맵 골격 |
| `<target>/.claude/DB_SCHEMA.md` | (선택) 설치된 DB 스키마 골격 |
| `<target>/.claude/DEPLOY.md` | (선택) 설치된 배포 정보 골격 |
| `<target>/.claude/DESIGN.md` | (선택) 설치된 디자인 시스템 골격 |
| `<target>/.claude/LESSONS.md` | (선택) 설치된 lesson 골격 |
