...
3. **agent-toolkit 스킬만 다룬다.** `${CLAUDE_PLUGIN_ROOT}/skills/`, `${CLAUDE_PLUGIN_ROOT}/skills-workflow/`, `${CLAUDE_PLUGIN_ROOT}/skills-system/` 세 디렉토리만 스캔한다. 외부 플러그인이나 사용자 글로벌 스킬은 대상이 아니다.
4. **임의 추천 금지.** 사용자 의도와 명확히 맞물리는 스킬만 추천하고, 매칭이 없으면 "해당 스킬 없음"이라고 명시한다.

## 사전 조건

- **플러그인 루트**: `${CLAUDE_PLUGIN_ROOT}` 환경변수로 접근. Claude Code가 이 플러그인의 skill을 실행할 때 자동 주입한다.
...
세 디렉토리의 모든 `SKILL.md`에서 frontmatter를 읽는다.

```bash
ROOT="${CLAUDE_PLUGIN_ROOT}"

for dir in skills skills-workflow skills-system; do
  find "$ROOT/$dir" -mindepth 2 -maxdepth 2 -name SKILL.md 2>/dev/null
done
```
...
다음 필드를 메모리에 보관한다:
- `category` — 디렉토리에서 도출: `skills/` = "독립", `skills-workflow/` = "워크플로우", `skills-system/` = "메타·스캐폴딩"
...
카테고리별로 분리한 세 개의 표를 출력한다. **description은 너무 길면 첫 문장 또는 80자로 자른다** (전체는 SKILL.md를 클릭해서 보라고 안내).

```markdown
## agent-toolkit 스킬 카탈로그

**플러그인 루트:** `<ROOT 절대경로>`
**총 개수:** N개 (독립 X · 워크플로우 Y · 메타 Z)

### skills/ — 독립 스킬

| 스킬 | 한 줄 설명 |
|---|---|
| [`name`](skills/name/SKILL.md) | description 첫 문장 또는 80자 컷 |
| ... | ... |
...
_(스킬이 없으면 "(현재 등록된 스킬 없음)" 한 줄로 대체)_
...
| 스킬 | 어떨 때 쓰는지 |
|---|---|
| [`name`](경로) | description의 "사용 시점" 또는 트리거 예시를 한 줄로 압축 (60~80자) |
| [`name`](경로) | ... |
| [`name`](경로) | ... |

### 상세 매칭

| 관련도 | 스킬 | 카테고리 | 매칭 이유 | 트리거 / 사용 시점 |
|---|---|---|---|---|
| 🟢 | [`name`](경로) | 독립 | "ERD" 키워드 직접 일치, "DB 설계 시각화" 목적 일치 | description의 트리거 예시 발췌 |
| 🟡 | [`name`](경로) | 메타 | "스키마" 동의어 매칭 | ... |
| ⚪ | [`name`](경로) | 독립 | 같은 DB 영역이지만 ERD가 아닌 스키마 탐색기 | ... |

### 매칭 없음 (해당하는 경우)

위 의도에 정확히 맞는 스킬이 없습니다. 가까운 영역의 스킬은 다음과 같습니다:
- (있으면 약한 관련만 나열)
- 아무것도 없으면 "agent-toolkit에 해당 스킬이 없습니다. `create-claude-plugin` 또는 직접 SKILL.md를 작성해서 추가하세요."
...
- `skills-system/create-claude-plugin` 참고 (플러그인 자체를 새로 만들 때)
...
| File | Purpose |
|------|---------|
| `${CLAUDE_PLUGIN_ROOT}/skills/*/SKILL.md` | 독립 스킬 카탈로그 원천 |
| `${CLAUDE_PLUGIN_ROOT}/skills-workflow/*/SKILL.md` | 워크플로우 스킬 카탈로그 원천 |
| `${CLAUDE_PLUGIN_ROOT}/skills-system/*/SKILL.md` | 메타·스캐폴딩 스킬 카탈로그 원천 |
