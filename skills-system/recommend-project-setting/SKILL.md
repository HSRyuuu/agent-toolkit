...
description: 이미 진행 중인 프로젝트의 현재 `.claude/` 세팅을 스캔하고, hsryuuu-toolkit이 제공하는 자산(스킬·작업 문서) 중 빠져 있거나 핵심 섹션이 누락된 항목을 찾아 사용자에게 추천합니다. **기본 동작은 read-only 추천 보고**이며, 사용자가 명시적으로 "설치/적용해줘"라고 의사를 밝히기 전까지는 대상 프로젝트의 어떤 파일도 변경하지 않습니다. 각 추천 항목에 대해 역할·필요한 세팅·이미 있는지 여부·대체 가능 여부·관련 스킬을 제시하고, 사용자가 명시적으로 설치를 요청하면 고른 항목만 설치합니다. project-setup이 빈 프로젝트에 골격을 까는 스킬이라면, 이 스킬은 진행 중인 프로젝트에 누락분을 보강하는 추천형 스킬입니다. 트리거 - "프로젝트 세팅 추천", ".claude 보강", "뭐 더 깔면 좋아?", "recommend setting", "/recommend-project-setting".
---

# Recommend Project Setting — 진행 중 프로젝트 세팅 추천

이미 어느 정도 세팅이 된 프로젝트를 스캔해서, hsryuuu-toolkit의 자산 중 **빠진 것 / 더 보강하면 좋은 것**을 사용자에게 알려주고, 사용자가 고른 것만 설치한다.

## 추천 자산 한눈에 보기

이 스킬이 분석·추천 대상으로 삼는 자산 전체 목록이다. 각 자산의 종류, 한 줄 설명, 적용 시 필요한 세팅을 요약한다.

| 자산 | 종류 | 한 줄 설명 | 적용 시 필요한 세팅 |
|---|---|---|---|
| `manage-skills` | 스킬 | 세션 변경사항 분석 → `verify-*` 스킬 생성/갱신 | `.claude/skills/manage-skills/` 디렉토리 추가 |
| `verify-implementation` | 스킬 | 등록된 모든 `verify-*` 스킬 통합 실행 | `.claude/skills/verify-implementation/` 디렉토리 추가 |
| `update-project-docs` | 스킬 | `.claude/` 작업 문서를 코드와 동기화 | `.claude/skills/update-project-docs/` 디렉토리 추가 |
| `CLAUDE.md` | 작업 문서 | 진입점 — 도메인 규칙·문서 매핑 | 루트 또는 `.claude/`에 파일 생성, 문서 매핑 표 채우기 |
| `PROJECT_OVERVIEW.md` | 작업 문서 | 정체성·기술 스택·마일스톤·핵심 링크 | `.claude/PROJECT_OVERVIEW.md` 파일 생성 후 스택·링크 채우기 |
| `SOURCE_MAP.md` | 작업 문서 | 소스코드 위치 라우팅 맵 | `.claude/SOURCE_MAP.md` 파일 생성 후 페이지·서비스 등록 |
| `DB_SCHEMA.md` | 작업 문서 | DB 테이블·컬럼·접근 범위 | `.claude/DB_SCHEMA.md` 파일 생성 후 테이블 등록 |
| `DEPLOY.md` | 작업 문서 | 인프라·도메인·환경변수·롤백 | `.claude/DEPLOY.md` 파일 생성 후 환경변수·인프라 채우기 |
| `DESIGN.md` | 작업 문서 | 디자인 시스템 — 색상·타이포·컴포넌트 | `.claude/DESIGN.md` 파일 생성 후 토큰·컴포넌트 채우기 |
| `LESSONS.md` | 자기개선 | 사용자 피드백 누적, 자기개선 순환 | `.claude/LESSONS.md` 파일 생성 + CLAUDE.md에 Self-Improving 섹션 한 줄 추가 |
...
### `update-project-docs`
- **역할**: `.claude/`의 작업 문서(CLAUDE.md, PROJECT_OVERVIEW.md, SOURCE_MAP.md, DB_SCHEMA.md, DEPLOY.md, DESIGN.md)가 코드 실제 상태와 어긋나면 동기화한다.
- **필요한 세팅**:
...
ls "$TARGET/.claude/PROJECT_OVERVIEW.md" 2>/dev/null
ls "$TARGET/.claude/SOURCE_MAP.md" 2>/dev/null
ls "$TARGET/.claude/DB_SCHEMA.md" 2>/dev/null
ls "$TARGET/.claude/DEPLOY.md" 2>/dev/null
ls "$TARGET/.claude/DESIGN.md" 2>/dev/null
...
이번 설치로 `.claude/`에 새로 들어온 자산이 있으면 CLAUDE.md "문서 매핑" 표에 행을 **추가**한다 (이미 있는 행은 건드리지 않음). CLAUDE.md를 이번에 새로 깔았다면 `project-setup`의 Step 8과 동일한 절차로 표 전체를 채운다.
...
| `<target>/.claude/skills/<key>/SKILL.md` | 설치 결과물 (스킬) |
| `<target>/.claude/<doc>` | 설치 결과물 (작업 문서·자기개선) |
