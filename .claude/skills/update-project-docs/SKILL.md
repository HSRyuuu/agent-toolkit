...
- `README.md` — 저장소를 처음 여는 사람을 위한 한 페이지짜리 진입점
...
- **skills/** — 독립 스킬 (현재 <N>개)
- **skills-workflow/** — 워크플로우 스킬 (현재 <N>개)
- **skills-system/** — 메타·스캐폴딩 스킬 (현재 <N>개)
...
- **개수는 catalog.md의 "요약" 표와 정확히 일치**해야 한다 (Step 2에서 만든 숫자 그대로).
- 길지 않게 — 한 화면 안에서 끝낸다.
