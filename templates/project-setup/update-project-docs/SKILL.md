...
다음 경우 즉시 종료한다:
- `CLAUDE.md`/`AGENTS.md`가 없음 → "진입점 문서를 먼저 설정하세요. `setup-agent-entrypoint` 참고."
...
1. **매핑 표가 비어 있음** — 정상 종료. 이 스킬은 새 문서를 만들지 않는다. 사용자가 `setup-agent-entrypoint` 또는 수동 편집으로 매핑을 채운 뒤 다시 실행해야 한다.
