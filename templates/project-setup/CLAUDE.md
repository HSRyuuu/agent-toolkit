...
- 사용자의 수정사항이 있을 경우 **docs/LESSONS.md** 에 정해진 패턴으로 업데이트하세요.
...
- 사소하지 않은 변경사항의 경우: 잠시 멈추고 **"더 나은 방법이 있을까?"**라고 자문해보세요.
- 임시 방편처럼 느껴지는 해결책인 경우: "지금 내가 아는 모든 것을 고려했을 때, 우아한 해결책을 실행하세요"
- 간단하고 명확한 해결책인 경우: 이 단계를 **건너뛰세요(Skip This Step)**. 지나치게 복잡하게 만들지 마세요.
- 최종 결과물을 보여주기 전에 자신의 작업물을 꼼꼼히 검토하세요.
